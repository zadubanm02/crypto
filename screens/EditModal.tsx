...
export const EditModal = ({visible, edit, closeModal, coin}) => {
    ...
    return (
        ...
        >
        <View style={{flex:1,backgroundColor:'#1F1D2B', paddingVertical:10}}>
            ...
            <View style={{marginTop:40}}>
                <TextInput onChangeText={(text:string)=>{setValue(parseFloat(text));dispatch(addCoin({...coin, value:parseFloat(text)}));setCoinToAdd({...coinToAdd,value:parseFloat(text)}) }} style={{backgroundColor:'#252836', padding:10, margin:10, color:'#fff', fontSize:24, borderRadius:15}}
                 placeholder='00.00' placeholderTextColor="#fff" keyboardType={'numeric'}/>
                 <TouchableOpacity onPress={()=>setModalVisible(true)} style={{backgroundColor:'#252836', padding:10, margin:10, borderRadius:15}}>
